...
public class OVRPlayerController : MonoBehaviour
{
    ...
    /// <summary>
    /// 두 컨트롤 방법을 혼합하여 사용할 떄 ture
    /// </summary>
    public bool CombineConstroll = false;
                ...
                if (CombineConstroll)
                {
                    if (OVRInput.Get(OVRInput.Button.SecondaryHandTrigger))
                    {
                        ...
                        if (secondaryAxis.x < 0.0f)
                            MoveThrottle += ort * (Mathf.Abs(secondaryAxis.x) * transform.lossyScale.x * moveInfluence *
                                                   BackAndSideDampen * Vector3.left);

                        if (secondaryAxis.x > 0.0f)
                            MoveThrottle += ort * (secondaryAxis.x * transform.lossyScale.x * moveInfluence * BackAndSideDampen *
                                                   Vector3.right);
                    }
                    else
                    {
